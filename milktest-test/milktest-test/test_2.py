...
def xgboost_parameter_tuning(*data):
    ...
    regr = xgb.XGBRegressor(n_estimators=260, max_depth=8, learning_rate=0.2, gamma=0, subsample=0.7,reg_alpha=0, reg_lambda=1)
    # regr = xgb.XGBRegressor(n_estimators=300, max_depth=3, learning_rate=0.1, gamma=0, subsample=0.8,reg_alpha=0, reg_lambda=1)
    regr.fit(X_train, y_train)
    joblib.dump(regr, 'train_model_fat.m')  # 保存模型
